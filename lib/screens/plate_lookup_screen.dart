...
class _PlateLookupScreenState extends State<PlateLookupScreen> {
  ...
  Future<void> _search() async {
    ...
    try {
      ...
      setState(() {
        _loading = false;
        _record  = found;
        _error   = found == null ? 'Vehicle not found. Please verify the plate number.' : null;
      });
      if (found != null) {
        await HistoryService.save(found);
      }
    } catch (e) {
      if (!mounted) return;
      setState(() { _loading = false; _error = 'An error occurred. Check your network.'; });
    }
  }
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: AppTheme.bgDeep,
        title: Text('Lookup & Pay', style: AppTheme.heading4.copyWith(letterSpacing: 1.2)),
        centerTitle: true,
        elevation: 0,
      ),
            ...
            child: Row(children: [
              ...
              Expanded(
                child: TextField(
                  ...
                  style: AppTheme.mono.copyWith(fontSize: 20, color: AppTheme.textPrimary),
                  decoration: InputDecoration(
                    hintText: 'RAC 001 A',
                    hintStyle: AppTheme.mono.copyWith(fontSize: 20, color: AppTheme.textHint),
